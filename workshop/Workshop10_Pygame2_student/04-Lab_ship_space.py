...
img_dir = path.join(path.dirname(__file__), 'source\img')
...
font = pg.font.Font(font_name, 50)  # กำหนดขนาด font
testX = 10
testY = 10
...
def displayText(text):
    font_name = pg.font.match_font('arial')  # กำหนดชื่อ Font
    font = pg.font.Font(font_name, 65)  # กำหนดขนาด font
    message = font.render(text, True, red)
    message_rect = message.get_rect()
    screen.blit(message, (120, 250))
...
while True:
    ...
    hits1 = pg.sprite.spritecollide(myShip, meteors, True)
    if len(hits1) > 0:
        import time
